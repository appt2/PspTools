...
public class ShortcutActivity extends Activity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (scoped) {
      ...
      SimpleFileChooser fileDialog =
          new SimpleFileChooser(
              this, Environment.getExternalStorageDirectory(), onFileSelectedListener);
    ...
    if (requestCode == RESULT_OPEN_DOCUMENT && data != null) {
      ...
      if (selectedFile != null) {
        ...
        if (Build.VERSION.SDK_INT >= 19) {
          ...
          getContentResolver()
              .takePersistableUriPermission(selectedFile, Intent.FLAG_GRANT_READ_URI_PERMISSION);
        }
  ...
  private void respondToShortcutRequest(Uri uri) {
      ...
      try {
        pathStr =
            java.net.URLDecoder.decode(
                segments[segments.length - 1], StandardCharsets.UTF_8.name());
      } catch (Exception e) {
    ...
    ShortcutIconResource iconResource =
        ShortcutIconResource.fromContext(this, R.drawable.ic_banner);
  ...
  private SimpleFileChooser.FileSelectedListener onFileSelectedListener =
      new SimpleFileChooser.FileSelectedListener() {
        @Override
