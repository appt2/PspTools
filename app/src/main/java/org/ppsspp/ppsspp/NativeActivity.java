...
public abstract class NativeActivity extends BaseCompat {
  ...
  private static final String[] permissionsForCamera = {Manifest.permission.CAMERA};
  private static final String[] permissionsForMicrophone = {Manifest.permission.RECORD_AUDIO};
  ...
  private void detectOptimalAudioSettings() {
    try {
      optimalFramesPerBuffer =
          Integer.parseInt(
              this.audioManager.getProperty(AudioManager.PROPERTY_OUTPUT_FRAMES_PER_BUFFER));
    } catch (NumberFormatException e) {
      // Ignore, if we can't parse it it's bogus and zero is a fine value (means we couldn't detect
      // it).
    }
    try {
      optimalSampleRate =
          Integer.parseInt(this.audioManager.getProperty(AudioManager.PROPERTY_OUTPUT_SAMPLE_RATE));
    } catch (NumberFormatException e) {
      // Ignore, if we can't parse it it's bogus and zero is a fine value (means we couldn't detect
      // it).
    }
      ...
      if (fileList != null) {
        for (File file : fileList) {
          if (!file.getAbsolutePath()
                  .equalsIgnoreCase(Environment.getExternalStorageDirectory().getAbsolutePath())
              && file.isDirectory()
              && file.canRead()) {
  ...
  private static ArrayList<String> getSdCardPaths19(final Context context) {
    ...
    if (externalCacheDirs == null || externalCacheDirs.length == 0) return null;
    if (externalCacheDirs.length == 1) {
      if (externalCacheDirs[0] == null) return null;
      final String storageState = Environment.getStorageState(externalCacheDirs[0]);
      if (!Environment.MEDIA_MOUNTED.equals(storageState)) return null;
      if (Environment.isExternalStorageEmulated()) return null;
    }
    final ArrayList<String> result = new ArrayList<>();
    if (externalCacheDirs.length == 1) result.add(getRootOfInnerSdCardFolder(externalCacheDirs[0]));
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    if (javaGL) {
      ...
      // It appears some gingerbread devices blow up if you use a config chooser at all ???? (Xperia
      // Play)
      // if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {

      // On some (especially older devices), things blow up later (EGL_BAD_MATCH) if we don't set
      // the format here, if we specify that we want destination alpha in the config chooser, which
      // we do.
      ...
      if (Build.MANUFACTURER.equals("OUYA")) {
        ...
        // Tried to mess around with config choosers (NativeEGLConfigChooser) here but fail
        // completely on Xperia Play.

        // Then I tried to require 8888/16/8 but that backfired too, does not work on Mali 450 which
        // is
  ...
  public static void gainAudioFocus(
      AudioManager audioManager, AudioFocusChangeListener focusChangeListener) {
  ...
  // keep this static so we can call this even if we don't
  // instantiate NativeAudioPlayer
  public static void loseAudioFocus(
      AudioManager audioManager, AudioFocusChangeListener focusChangeListener) {
    if (audioManager != null) {
  ...
  public static String getInputDesc(InputDevice input) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
      ...
      for (InputDevice.MotionRange range : motions) fakeid.append(range.getAxis());
    ...
    switch (keyCode) {
      case KeyEvent.KEYCODE_BACK:
        if (event.isAltPressed()) {
          NativeApp.keyDown(
              NativeApp.DEVICE_ID_PAD_0,
              1004,
              repeat); // special custom keycode for the O button on Xperia Play
      ...
      case KeyEvent.KEYCODE_DPAD_RIGHT:
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB_MR1
            && InputDeviceState.inputSourceIsJoystick(event.getSource())) {
          // Pass through / ignore
          return super.onKeyDown(keyCode, event);
        }
        // Fall through
      ...
      if (requestCode == RESULT_LOAD_IMAGE) {
        ...
        if (selectedDirectoryUri != null) {
          ...
          try {
            ...
            // Even if we got an exception getting permissions, continue and try to pass along the
            // file. Maybe this version of Android
        ...
        Toast.makeText(
                getApplicationContext(), "Bad request code: " + requestCode, Toast.LENGTH_LONG)
            .show();
  ...
  public void inputBox(
      final int requestId, final String title, String defaultText, String defaultAction) {
  ...
  public boolean processCommand(String command, String params) {
    ...
    if (command.equals("launchBrowser")) {
      ...
      try {
        ...
        || command.equals("browse_file_audio")
        || command.equals("browse_file_zip")) {
      try {
        int requestId = Integer.parseInt(params);
      ...
      inputMethodManager.toggleSoftInputFromWindow(
          surfView.getApplicationWindowToken(), InputMethodManager.SHOW_FORCED, 0);
      return true;
      ...
      if (param.length > 1 && param[1].length() > 0) title = param[1];
      if (param.length > 2) defString = param[2];
      ...
      if (surfView != null) {
        switch (milliseconds) {
          case -1:
            surfView.performHapticFeedback(
