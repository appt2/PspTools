...
public class PpssppActivity extends NativeActivity {
  ...
  public void onCreate(Bundle savedInstanceState) {
      ...
      new Thread() {
        ...
        public void run() {
          ...
          if (m_hasUnsupportedABI) {
            builder
                .setMessage(Build.CPU_ABI + " target is not supported.")
            ...
            builder
                .setMessage(
                    "The native part of PPSSPP for ABI "
                        + Build.CPU_ABI
                        + " is missing. Try downloading an official build?")
                .setTitle("Error starting PPSSPP")
                .create()
                .show();
          }
    ...
    if(getIntent().hasExtra("path")) {
     setShortcutParam(getIntent().getStringExtra("path"));
    }
  ...
  private String cursorToString(Cursor c) {
    ...
    if ((flags
            & (DocumentsContract.Document.FLAG_PARTIAL
  ...
  private long directorySizeRecursion(Uri uri) {
    ...
    try {
      ...
      // Buffer the URIs so we only have one cursor active at once. I don't trust the storage
      // framework
  ...
  public int contentUriCopyFile(String srcFileUri, String dstParentDirUri) {
    try {
      ...
      return DocumentsContract.copyDocument(getContentResolver(), srcUri, dstParentUri) != null
          ? STORAGE_ERROR_SUCCESS
          : STORAGE_ERROR_UNKNOWN;
    } catch (Exception e) {
  ...
  public int contentUriMoveFile(String srcFileUri, String srcParentDirUri, String dstParentDirUri) {
    try {
      ...
      return DocumentsContract.moveDocument(
                  getContentResolver(), srcUri, srcParentUri, dstParentUri)
              != null
          ? STORAGE_ERROR_SUCCESS
          : STORAGE_ERROR_UNKNOWN;
    } catch (Exception e) {
